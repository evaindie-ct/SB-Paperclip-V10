...
runtime_version: v10
source_manual: original/san-bernardo-paperclip-final-clean-operating-manual-with-diagram-and-librarian.md
worker_count_lossless: 37
worker_count_importable_default: 36
default_manifest: runtime/paperclip-runtime-manifest.importable-36.topological.json
paperclip_role_enum_safe: true
paperclip_runtime_root: ceo
---

# San Bernardo Agents — Paperclip Runtime v10

San Bernardo is a knitwear fashion brand business.

## Paperclip runtime structure

- **CEO** is the Paperclip runtime root.
- **Hermes Agent** reports to CEO.
- **Human Owner** is external governance/final approval, not an autonomous Paperclip worker.
- Every imported worker uses one of Paperclip's accepted API roles: `ceo`, `cto`, `cmo`, `cfo`, `engineer`, `designer`, `pm`, `qa`, `devops`, `researcher`, or `general`.
- The original business role is preserved as `business_role`, title, capabilities, and instruction text.
